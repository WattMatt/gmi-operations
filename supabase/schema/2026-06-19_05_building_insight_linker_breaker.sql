-- 2026-06-19_05: add circuit-breaker size to the building_insight_linker RPC shops[].
-- breaker_size lives in insight_linker.site_assets (per meter), NOT subsections, so we join
-- it per shop on (site_id, meter_serial_number) — the same meter that drives the Meter S/N
-- and CT columns. Shops without a meter serial (e.g. AbaQulusi, where metering is common-area)
-- get breaker_size = null → the UI renders "—", exactly like Meter S/N does today.
-- Requires 2026-06-19_04 (the site_assets foreign table). Everything else is unchanged from
-- 2026-06-18_09. Read-only, SECURITY DEFINER, gated by can_access_building().
...
begin
  ...
  docs as (
    select d.subsection_id,
           ...
           ) order by d.uploaded_at desc nulls last) as items,
           count(*) as n
  ...
  select jsonb_build_object(
    ...
    'shops', (
      select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'ct_ratio', m.ct_ratio,
          'breaker_size', (select sa.breaker_size from insight_linker.site_assets sa
                            where sa.site_id = v_site_id
                              and sa.meter_serial_number = m.meter_serial_number
                              and sa.breaker_size is not null
                            limit 1),
