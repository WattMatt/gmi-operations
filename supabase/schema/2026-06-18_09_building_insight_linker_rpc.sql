-- 2026-06-18_09: building-scoped, read-only RPC returning one shaped JSONB doc for the
-- "Electrical & Compliance" tab: site header, COC rollup, shops[] (electrical/COC +
-- matched GMI tenant + documents[] + photos[]), site documents.
-- SECURITY DEFINER so it can read insight_linker.* (which is OFF PostgREST); access is
-- gated by can_access_building(). insight-linker is read-only. Requires _05 + _08.
