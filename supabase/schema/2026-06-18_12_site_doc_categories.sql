-- 2026-06-18_12: surface the CATEGORY for site-level ("general") documents too — the files
-- not tied to a shop/tenant (insight-linker `site_documents`, joined by site_id). It already
-- carries a `category` text column (e.g. "08 Site Plan", "09 Generator", "02_Service records",
-- "08_Externally Generated Reports"); _08 only imported file_name/file_url. Add `category` to the
-- foreign table + the RPC's site_documents[] so the building tab's "Site documents" area shows
-- what each general file is. Read-only, re-runnable. (No NULL-subsection orphan docs exist, and
-- AbaQulusi has 0 site_documents — this benefits sites that do, e.g. Yarona/Mayville/Flamwood.)

-- Recreate the site-docs foreign table to ADD category (rest matches _08).
...
-- Replace building_insight_linker: identical to _11 EXCEPT site_documents[] now carries
...
begin
  ...
  select jsonb_build_object(
    ...
    'site_documents', (
      select coalesce(jsonb_agg(jsonb_build_object(
          'file_name', sd.file_name, 'file_url', sd.file_url, 'category', sd.category)
          order by sd.category nulls last, sd.file_name), '[]'::jsonb)
