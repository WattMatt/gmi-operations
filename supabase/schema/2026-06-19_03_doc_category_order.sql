-- 2026-06-19_03: surface the document category ORDER (document_categories.order_index) on each
-- per-shop document, so the building Documents tab can render categories in insight-linker's own
-- intended order (not just alphabetically by name). The web Documents tab now groups files as
-- Section (Building / Shop / Site-level) -> Category -> files; this gives it the exact category order.
--
-- Read-only, re-runnable. Identical to _12 EXCEPT the per-shop `documents[]` objects now also carry
-- 'category_order' (dc.order_index). site_documents has no order_index at source, so it keeps
-- ordering by category name. Requires _05 (server + user mapping) and the _11/_12 foreign tables.
